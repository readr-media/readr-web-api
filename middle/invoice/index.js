...
const createInvoice = data => new Promise((resolve, reject) => {
  const payload = {
    RespondType: 'JSON',
    Version: get(EZPAY, 'VERSION', '1.4'),
    TimeStamp: Math.round(Date.now() / 1000),
    MerchantOrderNo: get(data, 'transaction_id', `${Math.round(Date.now() / 1000)}`),
  ...
  switch (payload.TaxType) {
    case '2':
      ...
      /**
       * dont break and go next to get:
       * - 'TaxRate = 0'
       * - 'TaxAmt = 0'
       * - 'Amt = get(data, 'AmtSales', 0)'
       * - 'TotalAmt = get(data, 'AmtSales', 0)'
       */
    case '3':
      payload.TaxRate = 0
      payload.TaxAmt = 0
    ...
    default:
      ...
      payload.TaxAmt = Math.round(get(data, 'amtSales', 0) * (get(EZPAY, 'TAX_RATE', 5) / 100))
      if (payload.TaxType === '9') {
        payload.Amt = get(data, 'amtSales',  0) + get(data, 'amtZero',  0) + get(data, 'amtFree',  0) - payload.TaxAmt
        payload.TotalAmt = get(data, 'amtSales',  0) + get(data, 'amtZero',  0) + get(data, 'amtFree',  0)
        payload.ItemTaxType = map(get(data, 'items', []), item => get(item, 'taxType', 1)).join('|')
      } else {
  ...
  if (payload.Category === 'B2B') {
    if (!get(data, 'businessTitle') || !get(data, 'businessAddress') || !get(data, 'businessTaxNo')) {
    ...
    payload.ItemPrice = map(get(data, 'items', []), item => Math.round(get(item, 'price', 0) / (1 + payload.TaxRate / 100))).join('|')
    payload.ItemAmt = map(get(data, 'items', []), item => Math.round((get(item, 'price', 0) / (1 + payload.TaxRate / 100)) * get(item, 'count', 0))).join('|')
    if (payload.TaxType === '9') {
        payload.TaxType = '1'
    ...
    payload.ItemPrice = map(get(data, 'items', []), item => get(item, 'price', 0)).join('|')
    payload.ItemAmt = payload.ItemPrice
    if (get(data, 'loveCode')) {
      const exp_lovecode = /^[0-9]{3,7}$/
      if (exp_lovecode.test(get(data, 'loveCode', ''))) {
      ...
      switch (payload.CarrierType) {
        case '0':
        case '1':
          const exp_carrier_num = payload.CarrierType === CARRIER_TYPE.PHONE ? /^\/[A-Z0-9.+\-]{7}$/ : /^[A-Z]{2}[0-9]{14}$/
          if (!exp_carrier_num.test(get(data, 'carrierNum'))) {
            /**
             * 
             *  payload.CarrierType = CARRIER_TYPE.EXPAY
             *  payload.CarrierNum = get(data, 'member_mail', '-')
        ...
        case '2':
          ...
          payload.BuyerEmail = get(data, 'carrierNum', get(data, 'member_mail', '-')).trim()
          break
        default:
          delete payload.CarrierType
          payload.PrintFlag = 'Y'
...
const genInvoice = (req) => {
  ...
  .catch(error => {
    ...
    console.error(`Error occurred when gen inv for member-transaction:`, get(req, 'user.email', ''), '-', get(data, 'transaction_id', ''))
    console.error(err_wrapper.text)    
  })
}
